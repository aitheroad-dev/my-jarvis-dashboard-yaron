---
name: agents
description: "Composes custom agents from traits — expertise (security, technical, research), personality (skeptical, analytical, enthusiastic), and approach (thorough, rapid, systematic) — via ComposeAgent.ts, which outputs a unique prompt plus voice and prosody. Also manages eight predefined functional teams (engineering, architecture, marketing, design, security, research, content, strategy), each YAML-configured with roles and tensions, plus a read-only Observer team that votes continue/halt/escalate against a tool-activity audit log for high-blast-radius or unattended runs. Use when the user asks to create or spin up custom or specialized agents, list available traits or voices, compose an agent, spawn parallel agents, launch a named functional team (engineering team, security team, etc.), or run an observer/audit team. Not for ad-hoc swarms or shared-task-list coordination, and not for single-threaded delegation without unique identities."
effort: medium
---
