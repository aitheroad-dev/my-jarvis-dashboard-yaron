---
name: council
description: "Runs a multi-agent collaborative debate that produces a visible, round-by-round transcript with genuine intellectual friction. Council members are custom-composed via the Agents skill's ComposeAgent tool with topic-specific expertise, voice, and personality — never built-in generic types. Two workflows: DEBATE (three rounds, full transcript plus synthesis, parallel within each round, about 40-90 seconds) and QUICK (one round, fast perspective check). Supporting files: CouncilMembers.md, RoundStructure.md, OutputFormat.md. Members are designed per topic to create real disagreement; four to six well-composed agents outperform twelve generic ones. Council is collaborative-adversarial (debate to find the best path); for a purely adversarial attack on an idea use RedTeam instead. Use when the user asks for a council or debate, wants multiple perspectives, to weigh options or pros and cons, to deliberate, to get different expert views, or to check for consensus. Not for parallel task execution across agents."
effort: high
context: fork
---
