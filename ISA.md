...
## Decisions — open (2026-06-13, end of session)

- **Late-join gap (OPEN, no fix yet — user deferred):** auto-dispatch fires correctly at scheduled start (proven: bot 15209 then 15212), but a Vexa Google-Meet bot needs a human to ADMIT it, and Vexa pulls an alone/unadmitted bot after ~2 min. If the human joins late, the bot arrives to an empty room, leaves, and our own maybeEndMeeting (bot-gone detection) marks the meeting ended before the human arrives. Result: "the bot didn't join me." Proposed patience fix (NOT yet applied): raise DISPATCH_LEAD_MS, and suppress maybeEndMeeting's bot-gone end while now is inside the event's scheduled window (start..end) so the bot is re-dispatchable / not prematurely ended; optionally re-dispatch when participants are detected. Decide approach next session.
- Live state at session end: calendar connected (aitheroad@gmail.com); cron Worker my-jarvis-calendar-cron deployed (every minute), all secrets set; meeting id=8 bot_id=15212 status=live (test "Auto-join test", Meet kec-skja-bsm); dashboard commit ec9067d.
- Cleanup pending (non-blocking): delete duplicate Google OAuth client (id prefix 586267429205-dh8j…) — keep e2h0…; revoke spare Vexa keys created by double-clicks on the aitheroad Vexa account (that account's keys are NOT the working ones); add dashboard "create calendar event" button (event-creation proven via Calendar API, no UI yet).
